describe('Register Test',() => {
    it ('Registration test',() => {
        cy.fixture('Registration').then(data => {
            ...
            cy.log('Ввод пароля')
            cy.get(':nth-child(3) > .form-control--medium > .form-input--password').type(data.password_include_space_register)
            cy.log('Ввод пароля')
            cy.get(':nth-child(4) > .form-control--medium > .form-input--password').type(data.password_again_include_space_register)
        })
    })
})